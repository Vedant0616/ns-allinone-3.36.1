...
AthstatsHelper::EnableAthstats (std::string filename,  uint32_t nodeid, uint32_t deviceid)
{
  ...
  oss.str("");
  oss << "/NodeList/" << nodeid << "/DeviceList/" << deviceid << "/$ns3::WifiNetDevice/Phy/State/State";
  std::string statepath = oss.str ();
  Config::Connect (statepath, MakeCallback (&AthstatsWifiTraceSink::PhyStateTrace, athstats));
}
...
AthstatsWifiTraceSink::WriteStats ()
{
  ...
  snprintf (str, 200, "%8u %8u %7u %7u %7u %6u %6u %6u %7u %4u %3uM\n",
            ...
            (double) m_txDuration, // ast_tx_toobig
